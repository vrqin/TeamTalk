...
bool CGroupModel::isValidateGroupIdByDB(uint32_t nGroupId)
{
    bool bRet = false;
    CDBManager *pDBManager = CDBManager::getInstance();
    CDBConn *pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if (pDBConn)
    {
        ...
        if(bRet) {
            strSql = "select userId,updated from IMGroupMember where groupId=" + int2string(nGroupId) + " and status=0";
            ...
            if(pResultSet) {
                ...
                string strKey = "group_member_" + int2string(nGroupId);
                while(pResultSet->Next()){
                    uint32_t userId = pResultSet->GetInt("userId");
                    uint32_t updated = pResultSet->GetInt("updated");
                    pCacheConn->hset(strKey, int2string(userId), int2string(updated));
                }
                pCacheManager->RelCacheConn(pCacheConn);
                delete pResultSet;
            }
...
bool CGroupModel::isValidateGroupId(uint32_t nGroupId)
{
    ...
    if (pCacheConn)
    {
        ...
        if (!bRet)
        {
            bRet = isValidateGroupIdByDB(nGroupId);
        }
    }
    return bRet;
}
